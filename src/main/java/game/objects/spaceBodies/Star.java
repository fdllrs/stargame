...
public class Star extends SpaceBody implements Describable {
	...
	public Star(StarInfo info) {
		...
		Vector3f lightColor = new Vector3f(0.7f, 0.7f, 0.7f).add(colorA.mul(0.3f, new Vector3f()));
	...
	public List<Map.Entry<String, String>> getDisplayProperties() {
		return List.of(new AbstractMap.SimpleEntry<>("Type", starInfo.type().name()),
					   new AbstractMap.SimpleEntry<>("Radius",
													 String.format("%.1f", starInfo.radius())),
					   new AbstractMap.SimpleEntry<>("Mass",
													 String.format("%.1f", starInfo.mass())));
	}
	...
	public void render(ShaderProgram shader) {
		...
		float noiseScale =
				NOISE_SCALE_MIN + ( t * 0.5f + 0.5f ) * ( NOISE_SCALE_MAX - NOISE_SCALE_MIN );
