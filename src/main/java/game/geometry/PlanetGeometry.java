...
public class PlanetGeometry {
        ...
        for (Vector3f localUp : directions) {
            ...
            for (int y = 0; y <= resolution; y++) {
                for (int x = 0; x <= resolution; x++) {
                    constructVertex(resolution, radius, type, localUp, x, y, axisA, axisB, vertices, normals, colors);
    ...
    private static void constructVertex(int resolution, float radius, PlanetType type, Vector3f localUp,
                                        int x, int y, Vector3f axisA, Vector3f axisB,
                                        List<Float> vertices, List<Float> normals, List<Float> colors) {
    ...
    private static Vector3f calculateDisplacedNormal(Vector3f dir, float radius, PlanetType type, float height) {
        if (type != PlanetType.ROCKY && type != PlanetType.ORGANIC) {
    ...
    public static Mesh generateRing(float innerRadius, float outerRadius, int resolution) {
        ...
        for (int i = 0; i <= resolution; i++) {
            ...
            if (i < resolution) {
                ...
                indices.add(base + 1); indices.add(base + 2); indices.add(base + 3);
            }
        }
