...
import game.objects.spaceBodies.GasGiantPlanet;
...
public class GasGiantPlanetBuilder extends PlanetBuilder {
	...
	protected void generateColors() {
		...
		float s2 = Math.max(0.2f, Math.min(1.0f, s1 + ( RANDOM.nextFloat() * 0.3f - 0.15f )));

		float l1 = 0.25f + RANDOM.nextFloat() * 0.4f;
		float l2 = Math.max(0.15f, Math.min(0.85f, l1 + ( RANDOM.nextFloat() * 0.25f - 0.125f )));
