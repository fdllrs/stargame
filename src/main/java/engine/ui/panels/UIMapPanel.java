...
public class UIMapPanel extends UIPanel {
    ...
    @Override protected void rebuildElements() { /* procedural */ }

    @Override public float getBoundingHeight() {return height;}
    ...
    @Override public void render(ShaderProgram uiShader, Mesh uiQuad) {
        if (!visible)
            return;
    ...
    @Override protected void layout() { /* procedural */ }
    ...
    private static float calculateAdjustedDistance(float physDist,
        ...
        if (physDist < d0) {
            ...
            if (matchIdx != -1) {
                ...
                float dip1 = sortedPlanets.get(matchIdx + 1)
                                          .getPlanetInfo()
                                          .orbitDistance();
                float t = (physDist - di) / (dip1 - di);
                adjustedWorldDist = adjustedWorldRadii[matchIdx] + t *
                                                                   (adjustedWorldRadii[
                                                                            matchIdx +
                                                                            1] -
    ...
    private void renderOrbits(Mesh uiQuad,
                              Star star,
                              List<Planet> planets,
                              MapViewport vp) {
        ...
        for (Planet planet : planets) {
            float orbitRadius = getAdjustedDistance(planet.getPlanetInfo()
                                                          .orbitDistance(),
                                                    star,
                                                    vp.scale());
    ...
    private float getMoonOrbitRadius(Moon moon,
                                     Planet planet,
                                     float scale,
                                     float planetSize) {
    ...
    private void renderBodies(Mesh uiQuad,
                              Star star,
                              List<Planet> planets,
                              MapViewport vp) {
        for (Planet planet : planets) {
     ...
     */
    private record MapViewport(float centerX,
                               float centerY,
                               float scale,
                               UIMapPanel panel) {
        Vector2f toScreen(CelestialBody body, Star star) {
            if (body == star) {
                return new Vector2f(centerX, centerY);
            }
            if (body instanceof Planet planet) {
                if (planet instanceof Moon moon) {
                    float parentSz = panel.planetDisplaySize(moon.getParentPlanet(),
                                                             scale);
                    Vector2f parentPos = toScreen(moon.getParentPlanet(), star);
                    float dx = moon.getPosition().x -
                               moon.getParentPlanet().getPosition().x;
                    float dz = moon.getPosition().z -
                               moon.getParentPlanet().getPosition().z;
                    float dist = (float) Math.sqrt(dx * dx + dz * dz);
                    if (dist == 0f)
                        return parentPos;
